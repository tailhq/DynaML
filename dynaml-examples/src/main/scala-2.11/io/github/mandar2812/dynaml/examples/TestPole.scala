...
object TestPole {
  def apply(nCores: Int = 4, prototypes: Int = 1, kernel: String,
    ...
    val config = Map("file" -> "data/pole.test", "delim" -> ",",
      "head" -> "false",
      "task" -> "regression")

    val configtest = Map("file" -> "data/pole.data",
      "delim" -> ",",
      "head" -> "false")


    val conf = new SparkConf().setAppName("Pole Telecommunications").setMaster("local["+nCores+"]")
    ...
    val writer = CSVWriter.open(new File("data/resultsPole.csv"), append = true)
    writer.writeRow(row)
    writer.close()
    optModel.unpersist
    perf
