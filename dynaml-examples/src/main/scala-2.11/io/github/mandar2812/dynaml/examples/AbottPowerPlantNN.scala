...
object AbottPowerPlantNN {
  ...
  def apply(delta: Int, hidden: Int = 2,
            ...
            acts:List[String],
            num_training: Int = 2000, num_test: Int = 1000,
            stepSize: Double = 0.01,
            maxIt: Int = 300, mini: Double = 1.0,
            alpha: Double = 0.0,
            regularization: Double = 0.5): Unit =
    runExperiment(hidden, nCounts, acts,
      num_training, num_test, delta,
  ...
  def runExperiment(hidden: Int = 2,
                    nCounts:List[Int] = List(),
                    act: List[String],
                    num_training: Int = 200,
                    num_test: Int, deltaT: Int = 2,
    ...
    val modelTrainTest =
      (trainTest: ((Stream[(DenseVector[Double], DenseVector[Double])],
        ...
        val scoresAndLabelsPipe1 = DataPipe(
            ...
            DataPipe((res: Seq[(DenseVector[Double], DenseVector[Double])]) => {
              ...
              val outputAcc: List[ML[(Double, Double)]] = List(ML(), ML(), ML(), ML())
              res.foreach(r => {
        ...
        scoresAndLabels.foreach((output) => {
          ...
          legend(List(names(index), "Predicted "+names(index)+" (one hour ahead)"))
          title("Steam Generator; Abbott Power Plant, Champaign IL: "+names(index))
          unhold()
          index+=1
        })
