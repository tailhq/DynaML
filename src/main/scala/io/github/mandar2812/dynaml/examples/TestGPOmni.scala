...
/**
  ...
  */
object TestGPOmni {
  def apply(year: Int = 2006, kern: String = "RBF", bandwidth: Double = 0.5, noise: Double = 0.0,
            num_training: Int = 200, num_test: Int = 50): Unit = {

    val kernel: CovarianceFunction[DenseVector[Double], Double, DenseMatrix[Double]] =
      kern match {
        case "RBF" => new RBFKernel(bandwidth)
        case "Cauchy" => new CauchyKernel(bandwidth)
        case "Laplacian" => new LaplacianKernel(bandwidth)
      }
    ...
    val extractTrainingFeatures = (l: Stream[String]) => utils.extractColumns(l, ",", List(40,16,21,23,24),
