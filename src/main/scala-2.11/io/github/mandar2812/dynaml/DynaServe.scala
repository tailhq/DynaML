...
object DynaServe extends App with Service {
  ...
  val replServer = new DynaMLSSH(
    SshServerConfig(
      address = "localhost", // or "0.0.0.0" for public-facing shells
      port = 22222, // Any available port
      passwordAuthenticator = Some(new StaticPasswordAuthenticator(true))
      //publicKeyAuthenticator = Some(new KeySetPublickeyAuthenticator(Iterator.empty[PublicKey])) // or publicKeyAuthenticator
    )
  )
  replServer.start()
