...
import io.github.mandar2812.dynaml.repl.{Defaults, DynaMLRepl, Scripts}

/**
  * Contains the various entry points to the DynaML REPL.
  *
  ...
  *
  * It is more or less equivalent to the [[io.github.mandar2812.dynaml.repl.DynaMLRepl]]
  * ] object itself, and has
  * a similar set of parameters, but does not have any of the [[io.github.mandar2812.dynaml.repl.DynaMLRepl]]'s
  ...
  *           packages
  */
case class DynaML(predef: String = "",
  ...
  def runScript(path: Path,
    ...
    Scripts.runScript(wd, path, interp, args, kwargs)
  }
...
object DynaML {
  ...
  def main(args0: Array[String]) = {
    ...
    var replApi = false
    val replParser = new scopt.OptionParser[DynaML]("ammonite") {
    ...
    for(c <- replParser.parse(before, DynaML())) ifContinually(continually){
      def main(isRepl: Boolean) = DynaML(
        c.predef,
        c.defaultPredef,
