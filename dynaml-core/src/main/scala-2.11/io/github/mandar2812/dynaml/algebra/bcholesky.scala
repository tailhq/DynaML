...
object bcholesky extends UFunc {
  ...
  def choleskyAcc(mat: BlockedMatrix,
                  nAcc: Long,
  ...
  : RDD[((Long, Long), DenseMatrix[Double])] =
    mat.colBlocks*mat.rowBlocks match {
      ...
      * */
      case _ =>
        val L_11: BlockedMatrix =
          mat(0L to 0L, 0L to 0L).map(t => (t._1, inv(cholesky(t._2))))

        val tFunc: (((Long, Long), DenseMatrix[Double])) => ((Long, Long), DenseMatrix[Double]) =
          c => ((c._1._1+nAcc, c._1._2+nAcc), c._2)

        val L_21: BlockedMatrix = mat(1L until mat.rowBlocks, 0L to 0L)*L_11.t
  ...
  implicit object ImplCholesky_DM extends Impl[BlockedMatrix, BlockedMatrix] {
    def apply(X: BlockedMatrix): BlockedMatrix = {
      //Sanity Checks
      ...
      * */
      new BlockedMatrix(dat, X.rows, X.cols, X.rowBlocks, X.colBlocks)
    }
  }
}
