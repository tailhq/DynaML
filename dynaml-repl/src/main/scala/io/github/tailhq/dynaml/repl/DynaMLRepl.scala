...
under the License.
 * */
package io.github.tailhq.dynaml.repl
...
import io.github.tailhq.dynaml.repl.Router.{ArgSig, EntryPoint}
...
object Defaults {
  ...
  val welcomeBanner = {
    ...
    def banner =
      Source.fromFile(root_dir + "/conf/banner.txt").getLines.mkString("\n")
      ...
      scala.io.Source
        ...
        .map(l =>
          l.split(':') match {
            ...
            case other =>
              throw new Exception(
                s"Cannot parse line '$other' from resource $resourceName"
              )
          }
        )
    } finally {
  ...
  def dynaMlPredef =
    Source.fromFile(root_dir + "/conf/DynaMLInit.scala").getLines.mkString("\n")

}

object Compat {
  def copyAnnotatedType(
    c: Context
...
object Cli {
  ...
  case class Config(
    predefCode: String = Defaults.dynaMlPredef,
  ...
  def formatBlock(args: Seq[Arg[_, _]], leftMargin: Int) = {
    ...
    for (arg <- args) yield {
      showArg(arg).padTo(leftMargin, ' ').mkString +
        Predef
          .augmentString(arg.doc)
          .lines
          .mkString(Util.newLine + " " * leftMargin)
    }
  }
  def ammoniteHelp = {
          ...
          argsMap.get(realName) match {
            case Some(cliArg) =>
              if (cliArg.reader == scopt.Read.unitRead) {
                ...
                rest match {
                  case next :: rest2 =>
                    rec(rest2, cliArg.runAction(current, next))
...
object Router {
  ...
  object Overrides {
    ...
    def impl(c: Context): c.Tree = {
      import c.universe._
      q"new _root_.io.github.tailhq.dynaml.repl.Router.Overrides(${c.internal.enclosingOwner.overrides.length})"
    }
  }
  ...
  case class EntryPoint[T](
    ...
    def invoke(
      target: T,
      groupedArgs: Seq[(String, Option[String])]
    ): Result[Any] = {
      var remainingArgSignatures = argSignatures.toList

      val accumulatedKeywords =
        mutable.Map.empty[ArgSig[T], mutable.Buffer[String]]
      val keywordableArgs =
      ...
      if (
        ...
        val mapping = accumulatedKeywords.iterator.collect {
          case (k, b) if b.lengthCompare(1) == 0 => (k.name, b.head)
        }.toMap
  ...
  object Result {
    ...
    object Error {
      ...
      case class MismatchedArguments(
        missing: Seq[ArgSig[_]],
        unknown: Seq[String],
        duplicate: Seq[(ArgSig[_], Seq[String])],
        incomplete: Option[ArgSig[_]])
          extends Error
    ...
    object ParamError {
      ...
      /**
        ...
        */
      case class Invalid(arg: ArgSig[_], value: String, ex: Throwable)
          extends ParamError
  ...
  def makeReadVarargsCall[T: scopt.Read](
    arg: ArgSig[_],
    values: Seq[String]
  ) = {
...
class Router[C <: Context](val c: C) {
  ...
  def extractMethod(
    ...
    def unwrapVarargType(arg: Symbol) = {
      val vararg =
        arg.typeSignature.typeSymbol == definitions.RepeatedParamClass
      val unwrappedType =
    ...
    val readArgSigs =
      for (
        ...
        val argSig = q"""
        _root_.io.github.tailhq.dynaml.repl.Router.ArgSig(
          ${arg.name.toString},
      ...
      """
        ...
        val reader =
          if (vararg) q"""
          _root_.io.github.tailhq.dynaml.repl.Router.makeReadVarargsCall[$docUnwrappedType](
            $argSig,
            $extrasSymbol
          )
        """
          else q"""
        _root_.io.github.tailhq.dynaml.repl.Router.makeReadCall[$docUnwrappedType](
          $argListSymbol,
          $default,
          $argSig
        )
    ...
    q"""
    _root_.io.github.tailhq.dynaml.repl.Router.EntryPoint(
      ${meth.name.toString},
      scala.Seq(..$argSigs),
      ${methodDoc match {
      case None    => q"scala.None"
      ...
      ($baseArgSym: $curCls, $argListSymbol: Map[String, String], $extrasSymbol: Seq[String]) =>
        _root_.io.github.tailhq.dynaml.repl.Router.validate(Seq(..$readArgs)) match{
          case _root_.io.github.tailhq.dynaml.repl.Router.Result.Success(List(..$argNames)) =>
            _root_.io.github.tailhq.dynaml.repl.Router.Result.Success(
              $baseArgSym.${meth.name.toTermName}(..$argNameCasts)
            )
          case x: _root_.io.github.tailhq.dynaml.repl.Router.Result.Error => x
        },
      _root_.io.github.tailhq.dynaml.repl.Router.Overrides()
    )
    """
  }
...
object Scripts {
      ...
      processed <- interp.processModule(
        ...
        extraCode = Util.normalizeNewlines(
          s"""
            ...
            |object $$routes
            |extends scala.Function0[scala.Seq[_root_.io.github.tailhq.dynaml.repl.Router.EntryPoint[$$routesOuter.type]]]{
            |  def apply() = _root_.io.github.tailhq.dynaml.repl.Router.generateRoutes[$$routesOuter.type]
            |}
            """.stripMargin
        ),
        hardcoded = true
            ...
            scriptArgs match {
              ...
              case Seq((head, None), tail @ _*) =>
                mainMethods.find(_.name == head) match {
                  case None =>
                    Res.Failure(
                      s"Unable to find subcommand: " + backtickWrap(
                        head
                      ) + suffix
                    )
                  case Some(main) =>
  ...
  def formatMainMethodSignature[T](
    base: T,
...
private[dynaml] object ProxyFromEnv {
  ...
  /**
    ...
    */
  private def envToProps(env: (String, String)): Map[String, String] =
    env match {
